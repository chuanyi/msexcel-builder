...
(function() {
  ...
  Sheet = (function() {
    ...
    Sheet.prototype.cells = function(rows) {
      var c, cell, len, p, r, results, row;
      results = [];
      for (r = p = 0, len = rows.length; p < len; r = ++p) {
        row = rows[r];
        results.push((function() {
          var len1, q, results1;
          results1 = [];
          for (c = q = 0, len1 = row.length; q < len1; c = ++q) {
            cell = row[c];
            if (cell !== null && cell !== void 0) {
              results1.push(this.set(+c + 1, +r + 1, cell));
            } else {
    ...
    Sheet.prototype.text = function(col, row, str) {
    ...
    Sheet.prototype.merge = function(from_cell, to_cell) {
      var entry, len, p, results;
      if (Array.isArray(from_cell)) {
        results = [];
        for (p = 0, len = from_cell.length; p < len; p++) {
          entry = from_cell[p];
          results.push(this.merges.push(entry));
        }
        return results;
      } else {
    ...
    Sheet.prototype.width = function(col, wd) {
      var idx, len, p, results, width;
      if (Array.isArray(col)) {
        results = [];
        for (idx = p = 0, len = col.length; p < len; idx = ++p) {
          width = col[idx];
          if (idx > 0 && width) {
            results.push(this.width(idx, width));
          } else {
            results.push(void 0);
          }
        }
  ...
  Workbook = (function() {
    ...
    Workbook.prototype.worksheets = function(arr) {
      var idx, key, len, len1, ncols, nrows, p, q, ref, results, row, rowIdx, sheet, spec, val;
      results = [];
      for (idx = p = 0, len = arr.length; p < len; idx = ++p) {
        spec = arr[idx];
        nrows = spec.cells.length;
        ncols = 0;
        ref = spec.cells;
        for (rowIdx = q = 0, len1 = ref.length; q < len1; rowIdx = ++q) {
          row = ref[rowIdx];
          ncols = Math.max(ncols, row.length);
        }
        sheet = this.createSheet(spec.name, ncols, nrows);
        results.push((function() {
          var results1;
          results1 = [];
          for (key in spec) {
    ...
    Workbook.prototype.save = function(target, opts, cb) {
      ...
      if (!cb) {
        return new Promise(function(resolve, reject) {
          return self.save(function(err, zip) {
            if (err) {
              return reject(err);
            }
            return resolve(zip);
