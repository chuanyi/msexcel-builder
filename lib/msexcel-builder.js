/*
  MS Excel 2007 Creater v0.0.2
  Author : chuanyi.zheng@gmail.com
  History: 2012/11/07 first created
           2015/06/05 added support for numbers
           2015/06/09 added support for number formats
           2015/06/10 added support for dates
*/
...
tool = {
  ...
  jd2ed: function(jd) {
    var ed = 25568 + (jd.getTime() - (jd.getTimezoneOffset() * 1000 * 60)) / 1000 / 60 / 60 / 24;
    if ((jd.getFullYear() === 1900) && (jd.getMonth() === 0) && (jd.getDate() === 1)) {
      ed--;
    }
    if (ed > 59) {
      ed++;
...
Sheet = (function() {
  ...
  Sheet.prototype.set = function(col, row, value) {
    if ((typeof value) === 'string') {
      ...
      this.data[row][col].t = 's';
      return value
    }
    if ((typeof value) === 'number') {
      this.data[row][col].v = value;
      this.data[row][col].t = 'n';
      return value
    }
    if ((typeof value) === 'object') {
      if(Object.prototype.toString.call(value) === '[object Date]') {
        this.data[row][col].v = tool.jd2ed(value)
        this.data[row][col].t = 'n';
        return value
      }
    }
...
Style = (function() {
  ...
  Style.prototype.style2id = function(style) {
    ...
    k = 's_' +
        style.frmt_id + '_' + style.font_id + '_' + style.fill_id + '_' + style.bder_id + '_' +
        style.align + '_' + style.valign + '_' + style.wrap + '_' + style.rotate;
    id = this.cache[k];
    if (id) {
      return id;
    } else {
  ...
  Style.prototype.toxml = function() {
    var bders, cs, e, ea, es, fills, frmts, fonts, o, ss, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref, _ref2, _ref3, _ref4;
    ...
    for (_l = 0, _len4 = _ref4.length; _l < _len4; _l++) {
      ...
      if (o.frmt_id !== 1) e.att('applyNumberFormat', '1');
