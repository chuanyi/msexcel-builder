...
tool = {
  ...
  copy: function(origin, target) {
    ...
    if (path.existsSync(origin)) {
      if (!path.existsSync(target)) fs.mkdirSync(target, 0755);
...
Workbook = (function() {
  ...
  function Workbook(fpath, fname) {
    ...
    if (path.existsSync(target)) fs.rmdirSync(target);
  ...
  Workbook.prototype.save = function(cb) {
    ...
    exec.exec('zip -r ' + path.join(this.fpath, this.fname) + " * -x *.DS_Store", {cwd: target}, function(err, stdout, stderr) {
