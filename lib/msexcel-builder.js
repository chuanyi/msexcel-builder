      ...
      for (j = 1, _ref3 = this.cols; 1 <= _ref3 ? j <= _ref3 : j >= _ref3; 1 <= _ref3 ? j++ : j--) {
        ...
        if ((ix.v !== 0) || (sid !== 1)) {
          ...
          if (ix.v !== 0) {
            if (('' + ix.v).indexOf('=') === 0)
            {
              var str = '' + ix.v.substring(1);
              if (str.indexOf('~') !== -1)
              {
                c.ele('f', '' + str.substr(0, str.indexOf('~')));
                c.ele('v', '' + str.substr(str.indexOf('~') + 1, str.length - 1));
              }
              else
                c.ele('f', '' + str);
            }
            else if (('' + ix.v).indexOf('*') === 0)
            {
              c.ele('v', '' + ix.v.substring(1));
            }
            else
            {
              c.att('t', 's');
...
Style = (function() {
  ...
  Style.prototype.with_default = function() {
    ...
    this.def_numFmt_id = this.numFmt2id(null);
    this.def_align = '-';
    this.def_valign = '-';
    this.def_rotate = '-';
    this.def_wrap = '-';
  ...
  Style.prototype.numFmt2id = function(numFmt) {
    var id, k;
    numFmt || (numFmt = {});
    numFmt.format || (numFmt.format = '-');
    k = 'numFmt_' + numFmt.format;
    id = this.cache[k];
    if (id) {
      return id;
    } else {
      this.mnumFmts.push(numFmt);
      this.cache[k] = this.mnumFmts.length;
      return this.mnumFmts.length;
    }
  };
  ...
  Style.prototype.style2id = function(style) {
    ...
    k = 's_' + style.font_id + '_' + style.fill_id + '_' + style.bder_id + '_' + style.numFmt_id + '_' + style.align + '_' + style.valign + '_' + style.wrap + '_' + style.rotate;
    id = this.cache[k];
    if (id) {
      return id;
    } else {
  ...
  Style.prototype.toxml = function() {
    var bders, cs, e, ea, es, fills, fonts, numFmts, o, ss, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref0, _ref1, _ref2, _ref3, _ref4;
    ...
    numFmts = ss.ele('numFmts', {
      count: this.mnumFmts.length - 1
    });
    _ref0 = this.mnumFmts;
    for (_h = 1, _len = _ref0.length; _h < _len; _h++) {
      o = _ref0[_h];
      e = numFmts.ele('numFmt');
      if (o.format !== '-') e.att('formatCode', '' + o.format);
      e.att('numFmtId', '' + _h);
    }
    fonts = ss.ele('fonts', {
