...
(function() {
  var ContentTypes, DocPropsApp, SharedStrings, Sheet, Style, Workbook, XlRels, XlWorkbook, exec, fs, opt, os, tool, xml,
  ...
  Workbook = (function() {
    ...
    Workbook.prototype.save = function(cb) {
      var arg1, exe7z, i, opts, rmdirCmd, target, _i, _ref;
      target = this.fpath + '/' + this.id;
      ...
      return exec.exec(exe7z + arg1, opts, function(err, stdout, stderr) {
        if (err) {
          console.error('zip Error:' + err + ',' + stderr + ',' + target + ',' + arg1);
        }
        return exec.exec(rmdirCmd, function() {
          return cb(!err);
        });
      });
    };
