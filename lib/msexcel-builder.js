...
(function() {
  var ContentTypes, DocPropsApp, JSZip, SharedStrings, Sheet, Style, Workbook, XlRels, XlWorkbook, baseXl, opt, path, tool, xml,
  ...
  Workbook = (function() {
    function Workbook(fpath1, fname1) {
      ...
      this.saveSync = bind(this.saveSync, this);
    ...
    Workbook.prototype.saveSync = function() {
      return this._save(null, true);
    };

    Workbook.prototype.save = function(cb) {
      return this._save(cb, false);
    };

    Workbook.prototype._save = function(cb, sync) {
      var fs, target, write;
      fs = require('fs');
      write = sync ? fs.writeFileSync : fs.writeFile;
      target = this.fpath + path.sep + this.fname;
      return this.generate(function(err, zip) {
        ...
        if (sync) {
          return write(target, buffer);
        } else {
          return write(target, buffer, cb);
        }
      });
    };
